...
def is_ip_range(ip_range: str) -> bool:
    ...
    if seg_list[2].isdigit():
        ...
        return False
    range_list = seg_list[3].split("-")  # 第4段为 数字-数字 的形式，不能是单个数字
    if len(range_list) == 2:
...
def is_maskbyte(maskbyte: str) -> bool:
    ...
    """
    if not is_ip_addr(maskbyte):  # 子网掩码byte型 首先也是一个ip的形式，如果不是，则它不是一个子网掩码
        return False
    ...
    return maskbyte_to_int32 in maskbyte_set
...
def local__mask_seg_to_cidr(mask_seg: str) -> int:  # 已弃用
    """
...
if __name__ == '__main__':
    print("Hello, this is cofnet3.py")
