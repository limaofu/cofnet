...
if __name__ == '__main__':
    ...
    for ip_int in ip_int_list:  # 100万个ip的计算量，用时如下：
        ...
        try:
            ...
            # cofnet.is_ip_in_net_maskbyte(ip_address, "128.0.0.0", "128.0.0.0")  # 13.2 秒  ★★
            # cofnet.is_ip_in_range(ip_address,"10.99.1.1","10.99.1.255")  # 4.8 秒
            # cofnet.generate_ip_list_by_ip_range(ip_address + "-128")  # 34.2 秒   ★★★
            # ip_address2 = cofnet.int32_to_ip(ip_int+64)  # 1.0 秒  （这行和下面一行是一起的）
            # cofnet.generate_ip_list_by_ip_range_2(ip_address+"-"+ip_address2)  # 67.5 秒  （这行和上面一行是一起的） ★★★★★
            # system_id = cofnet.ip_to_net_system_id(ip_address)  # 3.2 秒
        except Exception:
            pass
        # print(ip_int)
        ...
        # cofnet.is_netseg_with_maskbyte(ip_address,"255.255.254.0") # 9.7 秒
